...
def draw_esdf_with_obstacle(ax, sdf, obstacle_mask, *, vmin=-1.0, vmax=4.0):
    cmap = plt.cm.get_cmap("RdYlGn").copy()
    cmap.set_bad(color=(0.05, 0.05, 0.05, 1.0))
    sdf_vis = np.ma.array(sdf, mask=obstacle_mask)
    ax.imshow(
        sdf_vis,
        cmap=cmap,
        vmin=vmin,
...
def compute_tangents(traj):
    tang = np.zeros_like(traj)
    tang[1:-1] = traj[2:] - traj[:-2]
    tang[0] = traj[1] - traj[0]
    tang[-1] = traj[-1] - traj[-2]
...
def compute_terms(
    ...
    # Two-circle car proxy centered around each trajectory point.
    tang = compute_tangents(traj_world)
    centers_front = traj_world + car_half_length * tang
    centers_rear = traj_world - car_half_length * tang

    d_front, g_front = query_sdf_points(
        centers_front, sdf, origin_xy, resolution
    )
    d_rear, g_rear = query_sdf_points(
        centers_rear, sdf, origin_xy, resolution
    )

    # Effective distance for finite-radius circle footprints.
    d_eff_front = d_front - car_radius
    d_eff_rear = d_rear - car_radius

    grad_edge_front = edge_target_grad(d_eff_front, g_front, d_target)
    grad_edge_rear = edge_target_grad(d_eff_rear, g_rear, d_target)
    grad_inside_front = inside_barrier_grad(
        d_eff_front, g_front, margin_inside
    )
    grad_inside_rear = inside_barrier_grad(
        d_eff_rear, g_rear, margin_inside
    )

    # Approximate center-point gradient as average of two circle-center terms.
    grad_edge = 0.5 * (grad_edge_front + grad_edge_rear)
    grad_inside = 0.5 * (grad_inside_front + grad_inside_rear)
    grad_smooth = smoothness_grad(traj_world)

    d_min_eff = np.minimum(d_eff_front, d_eff_rear)
    return d_min_eff, grad_edge, grad_inside, grad_smooth, centers_front, centers_rear
...
def total_cost(
    ...
    # edge target cost: minimum at d_eff = d_target (outside obstacle)
    edge_cost = 0.5 * np.sum((d_eff_front - d_target)**2)
    edge_cost += 0.5 * np.sum((d_eff_rear - d_target)**2)

    # inside barrier cost: active only if d_eff < margin_inside
    inside_mask_front = d_eff_front < margin_inside
    inside_mask_rear = d_eff_rear < margin_inside
    inside_cost = 0.5 * np.sum(
        (d_eff_front[inside_mask_front] - margin_inside)**2
    )
    inside_cost += 0.5 * np.sum(
        (d_eff_rear[inside_mask_rear] - margin_inside)**2
    )
...
def main():
    ...
    w_inside = 10.0       # strong inside penalty
    lambda_smooth = 0.05  # smoothness weight
    car_half_length = 0.08  # front/rear circle offset from center point
    car_radius = 0.10       # circle radius (covers half width + margin)
    ...
    _, grad_edge0, grad_inside0, grad_smooth0, _, _ = compute_terms(
        traj_world, sdf, origin_xy, resolution, d_target, margin_inside,
        car_half_length, car_radius
    )
    ...
    for k in range(iters):
        ...
        # ---- compute gradients
        d_vals, grad_edge, grad_inside, grad_smooth, _, _ = compute_terms(
            traj_world, sdf, origin_xy, resolution, d_target, margin_inside,
            car_half_length, car_radius
        )
        ...
        if (k + 1) % history_stride == 0:
            traj_history.append(traj_world.copy())
            _, grad_edge_snap, grad_inside_snap, grad_smooth_snap, _, _ = compute_terms(
                traj_world, sdf, origin_xy, resolution, d_target, margin_inside,
                car_half_length, car_radius
            )
        ...
        if k % 10 == 0:
            ...
            print(
                f"Iter {k}, min d_eff: {d_vals.min():.3f}, "
    ...
    draw_esdf_with_obstacle(ax, sdf, obstacle_mask, vmin=-1.0, vmax=4.0)
    ...
    # Show final two-circle car proxy for each waypoint.
    tang_final = compute_tangents(traj_final)
    front_final = traj_final + car_half_length * tang_final
    rear_final = traj_final - car_half_length * tang_final
    ...
    def draw_term_history(axh, grad_history, color, title):
        draw_esdf_with_obstacle(axh, sdf, obstacle_mask, vmin=-1.0, vmax=4.0)
